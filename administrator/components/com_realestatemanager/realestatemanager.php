...
function sendMailRentRequest($datas, $answer) {
    ...
    foreach ($datas as $key => $data) {
        ...
        mosMail($mosConfig_mailfrom, $from_name, $restemail,
...
function rent_requests($option, $bid) {
    ...
    HTML_realestatemanager :: showRequestRentHouses($option,
     $rent_requests, $h_associated, $title_assoc, $pageNav);
}
...
//**********************   end for reviews   *****************************//
    ...
    if (trim($house->id) != "") {
        ...
        foreach ($house_temp_photos as $house_temp_photo) {
            $house_photos[] = array($house_temp_photo->main_img, 
              picture_thumbnail($house_temp_photo->main_img, 
        ...
        if ($house_photo != '')
            $house_photo = array($house_photo, picture_thumbnail($house_photo, 
              $realestatemanager_configuration['foto']['high'],
               $realestatemanager_configuration['foto']['width']));
    }
    if (trim($bid) != "") {
    ...
    HTML_realestatemanager :: editHouse($option, $house, $clist, $ratinglist, $delete_edoc, 
      $reviews, $listing_status_list, $property_type_list, $listing_type_list, $house_photo,
...
/************************************    language     *********************/
    $associateArray = array();
    if($bid){
      $associateArray = edit_house_associate($house);
    }
/**************************************************************************/
    // get list of categories
    $categories = array();
...
//**********************   end for reviews   *****************************//
   ...
   if (trim($house->id) != "") {
        ...
        foreach ($house_temp_photos as $house_temp_photo) {
            $house_photos[] = array($house_temp_photo->main_img,
             picture_thumbnail($house_temp_photo->main_img, '150', '150'));
        }
        ...
        if ($house_photo != '')
            $house_photo = array($house_photo, picture_thumbnail($house_photo, '150', '150'));
    }
    if (trim($house->id) != "") {
...
function picture_thumbnail($file, $high_original, $width_original) {
    ...
    if (file_exists($mosConfig_absolute_path . '/components/com_realestatemanager/photos/'
     ...
     if($params3 == 1){ 
     $index = "_2_";
     $CreateNewImage = createImage($uploaddir.$file, $uploaddir.$file_name . $size
...
function configure_save_backend($option) {
    ...
    foreach($gtree as $g){
        $realestatemanager_configuration['user_manager_rem'][$g->value]['count_homes'] = 
          intval(mosGetParam($_POST, 'count_homes' . $g->value, "10"));
        $realestatemanager_configuration['user_manager_rem'][$g->value]['count_foto'] =
         intval(mosGetParam($_POST, 'count_foto' . $g->value, "10"));
    }
    ...
    $realestatemanager_configuration['review_email']['address'] =
            mosGetParam($_POST, 'review_email_address', ""); //back--
...
//configure_backend
    ...
    $lists['update'] = mosHTML :: RadioList($yesno, 'update', 'class="inputbox"',
     $realestatemanager_configuration['update'], 'value', 'text');
...
function orders($option) {
    ...
    if(isset($_REQUEST['order_details'])){
        ...
        $sql = "SELECT count(*)  ".
                ...
                " ON o.fk_house_id = g.id ". $order." ".$where ;
        $database->setQuery($sql);
        $total = $database->loadResult();
        $pageNav = new JPagination($total, $limitstart, $limit);
        $sql = "SELECT u.id as userId, o.name as user, u.username, ".
                       "o.*, ".
        ...
        $sql = "SELECT count(*)  ".
                ...
                " ON o.fk_house_id = h.id ". $order." ".
        $where ;
        $database->setQuery($sql);
        $total = $database->loadResult();
        $pageNav = new JPagination($total, $limitstart, $limit);
        $sql = "SELECT u.id as userId, o.name as user, u.username, ".
           "o.status, o.id, o.order_date, o.email,o.order_calculated_price, o.order_price, o.order_currency_code, ".
...
function updateOrderStatus() {
    ...
    $sql = "INSERT INTO `#__rem_orders_details`(fk_order_id,fk_user_id,status,order_date,fk_group_id,
...
function deleteOrder() {
  ...
  global $database;
    ...
    foreach($orderIds as $key=>$orderId){
        $sql = "DELETE FROM #__rem_orders WHERE id = ".$orderId." ";
        $database->setQuery($sql);
        $database->query();
