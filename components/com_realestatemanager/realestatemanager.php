...
require_once ($mosConfig_absolute_path .
 "/administrator/components/com_realestatemanager/images.php");
...
class PHP_realestatemanager {
    ...
    static function saveRentRequest($option, $bids) {  
        ...
        if(isset($_POST['user_email']) && $_POST['user_email'] != '') {
            ...
            $houseId = JRequest::getVar('houseid');
            ...
            $orderId= checkOrder($houseId);
            $sql = "UPDATE #__rem_orders SET fk_user_id = '".$user."',
                 status = 'Pending',
                 name = '".$name."',
         ...
         if(isset($_POST['customer_email']) && $_POST['customer_email'] != '') {
            ...
            if($realestatemanager_configuration['special_price']['show']){
                ...
                if($res){
                    ...
                    $sql = "SELECT price,priceunit FROM #__rem_houses WHERE id='".$houseId."'";
                    $database->setQuery($sql);
                    $res = $database->loadObjectList();
                }
            }else{
...
/**********************************************/
        ...
        HTML_realestatemanager::displayHouse($house, $tabclass,
             $params, $currentcat, $ratinglist, $house_photos, $id, $catid,
    ...
    static function showSearchHouses($options, $catid, $option, $layout = "") {
        ...
        $listing_type_list = mosHTML :: selectList($listing_type, 'listing_type',
         'class="inputbox" size="1" style="width: 115px"', 'value', 'text', _REALESTATE_MANAGER_LABEL_ALL);
        ...
        $property_type_list = mosHTML :: selectList($property_type, 'property_type', 'class="inputbox"
           size="1" style="width: 115px"', 'value', 'text', '');
        ...
        $clist = PHP_realestatemanager::com_house_categoryTreeList(0, '', true, $categories, $catid, $lang);
...
//**************   end show search_option     ******************************        
    ...
    static function searchHouses($options, $catid, $option, $languagelocale, $ownername = '') {
        ...
        $pricefrom = intval(mosGetParam($_REQUEST, 'pricefrom2', ''));
        $priceto = intval(mosGetParam($_REQUEST, 'priceto2', ''));
        if ($pricefrom > 0)
        ...
        if (isset($_REQUEST['search_date_from']) && (trim($_REQUEST['search_date_from']) ) &&
            ...
            $RentSQL = "((fk_rentid = 0 OR b.id NOT IN (select dd.fk_houseid " .
             ...
             "' and dd.fk_houseid=b.id)) AND (listing_type = \"1\"))";
            if ($is_add_or)
                $RentSQL .= " AND ";
        ...
        if (isset($_REQUEST['search_date_until']) && (trim($_REQUEST['search_date_until']) )
         && trim($_REQUEST['search_date_from']) == "") {
            $RentSQL = "((fk_rentid = 0 OR b.id NOT IN (select dd.fk_houseid "
             ...
             . $search_date_until . "' and dd.fk_houseid=b.id)) AND (listing_type = \"1\"))";
            if ($is_add_or)
                $RentSQL .= " AND ";
            $RentSQL_JOIN_1 = "\nLEFT JOIN #__rem_rent AS d ";
            $RentSQL_JOIN_2 = "\nON d.fk_houseid=b.id ";
          ...
          if ( $task == 'my_houses'
            || $task == 'show_my_houses' || $task == 'showmyhouses'  )
              ...
              print_r("<h1 style='text-align:center'>" . _REALESTATE_MANAGER_LABEL_SEARCH_NOTHING_FOUND .
               " </h1><br><br><div class='row-fluid'><div class='span9'></div>
...
static function ajax_rent_calcualete($bid,$rent_from,$rent_until){ 
     ...
     */
    static function picture_thumbnail($file, $width_original, $high_original) {
        ...
        if (file_exists($mosConfig_absolute_path . '/components/com_realestatemanager/photos/'
         ...
         if($params3 == 1){
         $index = "_2_";
         $CreateNewImage = createImage($uploaddir.$file, $uploaddir.$file_name . $size . $index .
        ...
        switch (strtolower($file_type)) {
            case '.png':
                ...
                imagepng($tn, $mosConfig_absolute_path . '/components/com_realestatemanager/photos/'
                 . $file_name . $size . $index . $file_type);
                break;
            case '.jpg':
                $source = imagecreatefromjpeg($mosConfig_absolute_path . '/components/com_realestatemanager/photos/' . $file);
                $file = imagecopyresampled($tn, $source, 0, 0, 0, 0, $w_, $h_, $width, $height);
                imagejpeg($tn, $mosConfig_absolute_path . '/components/com_realestatemanager/photos/'
                 . $file_name . $size . $index . $file_type);
                break;
            case '.jpeg':
                $source = imagecreatefromjpeg($mosConfig_absolute_path . '/components/com_realestatemanager/photos/' . $file);
            ...
            case '.gif':
                ...
                $file = imagecopyresampled($tn, $source, 0, 0, 0, 0, $w_, $h_, $width, $height);
                imagegif($tn, $mosConfig_absolute_path . '/components/com_realestatemanager/photos/'
                 . $file_name . $size . $index . $file_type);
                break;
            default:
                echo 'not support';
    ...
    static function com_house_categoryTreeList($id, $action, $is_new, &$options = array(), $catid='', $lang='') {
        global $database;
        if($lang){
            $list = PHP_realestatemanager::com_house_categoryArray($lang);
        } else {
    ...
    static function com_house_categoryArray($lang='') {
